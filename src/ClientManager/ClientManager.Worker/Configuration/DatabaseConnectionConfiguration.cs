...
public class DatabaseConnectionConfiguration()
{
    ...
    /// <returns></returns>
    public static DatabaseConnectionConfiguration Load(string? basePath = null)
    {
        ...
        {
            Console.WriteLine(
                $"Warning: .env file not found at {envPath}. Using appsettings.json for environment variables or default database configuration."
            );
        }
        ...
        return new DatabaseConnectionConfiguration
        {
            ...
            Port = int.TryParse(
                Environment.GetEnvironmentVariable("POSTGRES_PORT")
                    ?? configuration["DatabaseConfig:POSTGRES_PORT"],
                out var port
            )
                ? port
                : 5432,
